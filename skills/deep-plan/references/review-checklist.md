...
- [ ] Every Phase 0 **Research Question** is answered with evidence or tagged
      UNVERIFIED — none silently dropped.
...
- [ ] Every decision carries a **REVERSIBLE / ONE-WAY** tag; every ONE-WAY
      decision was user-confirmed at the Decision Brief Gate or is explicitly
      marked UNCONFIRMED.

## Roadmap

- [ ] Roadmap Phase 0 is shippable **security/correctness** fixes, not features.
- [ ] If new functionality is built, the first phase after Phase 0 is a
      **walking skeleton** (thinnest end-to-end runnable slice), not a
      layer-by-layer buildout.
- [ ] Every **UNVERIFIED** tag maps to a named **spike task**, placed first
      in the roadmap phase that depends on it.
...
- [ ] Every checklist item names concrete files/classes/migrations — no
      "improve system X".
- [ ] A risk register exists with mitigation/rollback per risk, generated by
      **pre-mortem** — no generic "X might fail" entries that fit any plan.
